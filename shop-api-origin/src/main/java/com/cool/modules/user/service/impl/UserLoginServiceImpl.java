...
public class UserLoginServiceImpl implements UserLoginService {
    ...
    public Object refreshToken(String refreshToken) {
        ...
        CoolPreconditions.check(jwt == null || !(Boolean)jwt.getPayload("isRefresh"), "错误的refreshToken");
    ...
    private Object wxLoginToken(UserWxEntity userWxEntity) {
        String unionId =
            ObjUtil.isNotEmpty(userWxEntity.getUnionid()) ? userWxEntity.getUnionid() : userWxEntity.getOpenid();
        UserInfoEntity userInfoEntity =
            userInfoService.getOne(QueryWrapper.create().eq(UserInfoEntity::getUnionid, unionId));
        if (ObjUtil.isEmpty(userInfoEntity)) {
            userInfoEntity = new UserInfoEntity();
            userInfoEntity.setNickName(
                ObjUtil.isNotEmpty(userWxEntity.getNickName()) ? userWxEntity.getNickName() : generateRandomNickname());
    ...
    public Object password(String phone, String password) {
        UserInfoEntity userInfoEntity =
            userInfoService.getOne(QueryWrapper.create().eq(UserInfoEntity::getPhone, phone));
    ...
    private Dict generateToken(UserInfoEntity userInfoEntity, String refreshToken) {
        Dict tokenInfo = Dict.create().set("userType", UserTypeEnum.APP.name()).set("userId", userInfoEntity.getId());
        ...
        return Dict.create().set("token", token).set("expire", jwtTokenUtil.getExpire())
            .set("refreshToken", refreshToken).set("refreshExpire", jwtTokenUtil.getRefreshExpire());
    }
}
