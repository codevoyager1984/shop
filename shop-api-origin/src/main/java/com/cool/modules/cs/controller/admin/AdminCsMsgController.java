...
public class AdminCsMsgController extends BaseController<CsMsgService, CsMsgEntity> {
    @Override
    protected void init(HttpServletRequest request, JSONObject requestParams) {
        String sessionId = requestParams.get("sessionId", String.class);
        setPageOption(
            createOp().select(CS_MSG_ENTITY.ALL_COLUMNS,
                ...
                .on(CS_MSG_ENTITY.USER_ID.eq(USER_INFO_ENTITY.ID).and(CS_MSG_ENTITY.TYPE.eq(0)))
                .leftJoin(BASE_SYS_USER_ENTITY)
                .on(CS_MSG_ENTITY.USER_ID.eq(BASE_SYS_USER_ENTITY.ID).and(CS_MSG_ENTITY.TYPE.eq(1)))
                .and(CS_MSG_ENTITY.SESSION_ID.eq(sessionId))));
    }
